...
parser.add_argument(
    '--list_ResBlock_inplanes', type=list, default=[4,],
...
parser.add_argument(
    '--path_reference_set', type=str, default='/home/DM2/workspace/SPEMR/data/LAMOST_DR8_APOGEE_DR17/refer_set/',
...
def train(args, dataset_info, train_label=['Teff[K]', 'Logg', 'FeH'], model_number="SP0", cuda=True):
    ...
    for epoch in tqdm(range(1, args.n_epochs + 1)):
        ...
        print("EPOCH %d | lr %f | train_loss %.4f | valid_loss %.4f" % (epoch, lr, train_loss, valid_loss),
              "| valid_mae %.5f", valid_mae,
              "| valid_diff_std %.5f", vlaid_diff_std)
...
#               model_number="SP%d" % i)
    for i in range(3, 6):
        train(args, dataset_info=dataset_info,
              train_label=['ASPCAP_CH', 'ASPCAP_NH', 'ASPCAP_OH',
       'ASPCAP_MgH', 'ASPCAP_AlH', 'ASPCAP_SiH', 'ASPCAP_SH', 'ASPCAP_KH',
       'ASPCAP_CaH', 'ASPCAP_TiH', 'ASPCAP_CrH', 'ASPCAP_MnH', 'ASPCAP_NiH'],
              model_number="CA%d" % i)
